...
from Functions import Styblinski_Tang_Function, Styblinski_Tang_Function_Limits
...
axis = np.linspace(Styblinski_Tang_Function_Limits[0], Styblinski_Tang_Function_Limits[1], num_grid_points_each_axis)

def target_function(matrix) :

    return Styblinski_Tang_Function(matrix[0:1, :], matrix[1:, :])
...
for x in axis :
    ...
    for y in axis :
       ...
       a0[0].append(x)
       a0[1].append(y)
       t[0].append(Styblinski_Tang_Function(x,y))
...
random_points = Styblinski_Tang_Function_Limits[0] + (Styblinski_Tang_Function_Limits[1] - Styblinski_Tang_Function_Limits[0]) * np.random.rand(2, 40)
np.savetxt('Styblinski Tang Function Approximation Test Points.csv', random_points.transpose(), delimiter=',')
...
anim.save('Styblinski Tang Training Model.mp4', writer='ffmpeg', fps=30)